...
export function renderReviewPage(candidates: Candidate[], rootDir: string): string {
  ...
  const initialData = JSON.stringify(
    [...byFile.entries()].map(([file, items]) => ({
      ...
      absFile: file,
      items: items.map((c) => ({ id: c.id, original: c.original, converted: c.converted })),
    })),
  );
...
<head>
  ...
  <style>
    ...
    .file-header { background: #f7f7fb; padding: 0.6rem 0.9rem; font-weight: 600; font-family: monospace; font-size: 0.9rem; display: flex; justify-content: space-between; align-items: center; }
  ...
  <script>
    ...
    function render() {
      ...
      for (const group of data) {
        ...
        fileDiv.innerHTML = \`<div class="file-header"><span>\${group.file}</span><span>\${group.items.length} ta</span></div>\`;
        for (const item of group.items) {
          const row = document.createElement("label");
          row.className = "item";
          row.innerHTML = \`
            <input type="checkbox" \${approved.has(item.id) ? "checked" : ""} data-id="\${item.id}" />
